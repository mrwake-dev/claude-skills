...
- **Fan-out** — anything larger: launch parallel review subagents (Agent tool, `general-purpose`), **all in a single message** so they run concurrently — one per applicable dimension. Set each spawn's `description` parameter to the **Agent label** from its row: `general-purpose` is the agent *type* and is identical for all of them, so without a per-dimension label the running agents are indistinguishable — and when one fails its retry, the Step 4.4 coverage line has to name *which* dimension went unreviewed.

| Dimension                                                         | Agent label                    | Rule file(s)                                                                             | When                                 |
|:------------------------------------------------------------------|:-------------------------------|:-----------------------------------------------------------------------------------------|:-------------------------------------|
| Correctness & code quality (incl. tests, dependencies, CHANGELOG) | `Correctness & quality review` | `references/general.md` except the Security and Documentation sections                   | always                               |
| Security                                                          | `Security review`              | `references/general.md` — Security section                                               | always                               |
| Documentation & comments                                          | `Docs & comments review`       | `references/general.md` — Documentation & comments section                               | always                               |
| Language conventions                                              | `<Language> conventions review` — name the detected language, e.g. `TypeScript conventions review` | the detected `references/typescript.md` / `react.md` / `java.md` / `python.md` / `go.md` | per detected language                |
| Device runtime                                                    | `Device runtime review`        | `references/device-runtime.md`                                                            | when the Step 2 device triggers match |
